...
#define nan_check_deep(list) for(auto& _nan : ((list))){cpp_assert(std::isfinite(_nan), "NaN Verify");}
#define nan_check_deep_deep(l) for(auto& _nan_a : ((l))){for(auto& _nan_b : _nan_a){cpp_assert(std::isfinite(_nan_b), "NaN Verify");}}
