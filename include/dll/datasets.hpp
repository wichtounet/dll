...
auto make_mnist_generator_train(const std::string& folder, size_t limit, size_t batch, Parameters&&... /*parameters*/){
    ...
    if(!mnist::read_mnist_image_file_flat(generator->input_cache, folder + "/train-images-idx3-ubyte", m)){
    ...
    if(!mnist::read_mnist_label_file_categorical(generator->label_cache, folder + "/train-labels-idx1-ubyte", m)){
 ...
 */
template<typename... Parameters>
auto make_mnist_generator_test(const std::string& folder, size_t limit, size_t batch, Parameters&&... /*parameters*/){
    // Create examples for the caches
    etl::fast_dyn_matrix<float, 1, 28, 28> input;
    float label;
    ...
    // Read all the necessary images
    if(!mnist::read_mnist_image_file_flat(generator->input_cache, folder + "/t10k-images-idx3-ubyte", m)){
    ...
    // Read all the labels (categorical)
    generator->label_cache = 0;
    if(!mnist::read_mnist_label_file_categorical(generator->label_cache, folder + "/t10k-labels-idx1-ubyte", m)){
 ...
 */
template<typename... Parameters>
auto make_mnist_generator_train(size_t limit = 0, size_t batch = 0, Parameters&&... parameters){
    return make_mnist_generator_train("mnist", limit, batch, std::forward<Parameters>(parameters)...);
}
 ...
 */
template<typename... Parameters>
auto make_mnist_generator_test(size_t limit = 0, size_t batch = 0, Parameters&&... parameters){
    return make_mnist_generator_test("mnist", limit, batch, std::forward<Parameters>(parameters)...);
}
