...
struct avgp_layer_2d final : pooling_layer_2d<avgp_layer_2d<Desc>, Desc> {
    using desc   = Desc;                                        ///< The layer descriptor
    using weight = typename desc::weight;                       ///< The layer weight type
    using base   = pooling_layer_2d<avgp_layer_2d<Desc>, desc>; ///< The layer base type
    ...
    static void activate_hidden(output_one_t& h, const Input& v) {
        h = etl::avg_pool_2d<base::C1, base::C2>(v);
    }
    ...
    void backward_batch(H&& output, C& context) const {
        ...
        output = etl::avg_pool_derivative_2d<C1, C2>(context.input, context.output) >> etl::upsample_2d<C1, C2>(context.errors);
    }
...
template<typename Desc>
struct layer_base_traits<avgp_layer_2d<Desc>> {
...
struct sgd_context<DBN, avgp_layer_2d<Desc>, L> {
    ...
    sgd_context(avgp_layer_2d<Desc>& /*layer*/){}
};
