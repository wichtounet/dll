...
struct input_converter {
    ...
    input_converter(DBN& dbn, const Iterator& first, const Iterator& last)
            : c(dbn.template layer_get<I>().convert_input(first, last)) {
...
struct input_converter<DBN, I, Iterator, std::enable_if_t<
                                             std::is_same<typename DBN::template layer_type<I>::input_one_t, typename Iterator::value_type>::value && !layer_traits<typename DBN::template layer_type<I>>::is_multiplex_layer()>> {
    Iterator& first;
    Iterator& last;

    input_converter(DBN& /*dbn*/, Iterator& first, Iterator& last)
...
template <typename DBN, std::size_t I, typename Iterator>
struct input_converter<DBN, I, Iterator, std::enable_if_t<layer_traits<typename DBN::template layer_type<I>>::is_transform_layer()>> : input_converter<DBN, I + 1, Iterator> {
...
struct sample_converter {
    ...
    sample_converter(const DBN& dbn, const Sample& sample)
            : r(dbn.template layer_get<I>().convert_sample(sample)) {
        //Nothing else to init
    }
...
struct sample_converter<DBN, I, Sample, std::enable_if_t<
                                            std::is_same<typename DBN::template layer_type<I>::input_one_t, Sample>::value && !layer_traits<typename DBN::template layer_type<I>>::is_multiplex_layer()>> {
    const Sample& r;
...
template <typename DBN, std::size_t I, typename Sample>
struct sample_converter<DBN, I, Sample, std::enable_if_t<layer_traits<typename DBN::template layer_type<I>>::is_transform_layer()>> : sample_converter<DBN, I + 1, Sample> {
